function [inerr, dw, inLL, right] = batch_cell_lstm(W, dataall, maskall, problem_name)

    % gradient threshold for bptt need done, inLL, right

    global mzeros convert usegpu problem_midline problem_motion signum

    if strcmp(problem_name, 'fish_midline') == 1
        in_size = problem_midline.in_size;
        gate_size = problem_midline.gate_size;
        out_size = problem_midline.out_size;
        share_size = problem_midline.share_size;
        numMmcell = problem_midline.numMmcell;
        share_size2 = problem_midline.share_size2;
        in = problem_midline.in;
        ingate = problem_midline.ingate;
        cellstate = problem_midline.cellstate;
        cells = problem_midline.cells;
        outgate = problem_midline.outgate;
        node_outgateInit = problem_midline.node_outgateInit;
        cellinInit = problem_midline.cellinInit;
        node_cellbiasInit = problem_midline.node_cellbiasInit;
        delta_outInit = problem_midline.delta_outInit;
        cellstatusInit = problem_midline.cellstatusInit;
    else
...
% % %             node_out = [node_cell{t} node_cellbias{t}] * Wout;
            
% % %             delta_out{t} = (-output + node_out);
%             delta_out{t} = (2 * [node_cell{t} node_cellbias{t}]' * [node_cell{t} node_cellbias{t}] * Wout - 2 * [node_cell{t} node_cellbias{t}]' * output) ./ size(node_out, 1);
...
%     size(Wout)
    ...
    delta_outgate{t} = squeezing(delta_fun(tmp_outgate, signum) .* errorcell .* Y_cellout{t});
    ...
    for t = timespan - 1 : -1 : 2
        ...
        errorcell = delta_out{t} * Wout(1 : end - 1, :)' + delta_outgate{t + 1} * Woutgate_cell' + ...
            delta_cellin{t + 1} * Wcell_cell' + delta_ingate{t + 1} * Wingate_cell';
        ...
        delta_outgate{t} = delta_fun(node_outgate{t}, signum) .* (squeezing(errorcell .* Y_cellout{t}) + ...
            ...
            delta_outgate{t + 1} * Woutgate_outgate');
